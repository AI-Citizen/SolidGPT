...
class HTTPCodePlan(WorkSkill):
    ...
    def __init__(self):
        ...
        self.name = SKILL_NAME_HTTP_CODE_PLAN
    ...
    def _read_input(self):
        ...
        self.cur_dir = os.path.join(LOCAL_STORAGE_OUTPUT_DIR, self.session_id, "architecture")
    ...
    def __build_plan_prompt(self):
        return f"""Assume you are Senior Software engineer. Given Existing Code Files and Requirement, design a detailed code implementation plan. Follow these attentions and rules:
                ...
                5.Design a NoSQL collection schema that satisfy the user requirement. You must only design ONE collection. Be concise, only includes the necessary properties in the collection. 

                6.Change the directory name under "{self.cur_dir}" from "todos" to the collection name you just created.
                
                7.Change all the fields name from "todos" to the collection name you just created in serverless.yml
    ...
    def __build_dev_design_prompt(self):
        return f"""Assume you are Senior Software engineer. Given the summary of code base and Requirement, design a high level design of code implementation. Follow these attentions and rules:
                ATTENTION: Description in the each plan step should be clear, and very detailed. It should be easy for other software engineers to understand and follow to implement.
                ATTENTION: Before starting, decide how many files are needed to satisfy the requirement. You should only choose files from EXISTING FILES.
                RULES:
                1.Check Project Summary, think about the relationship between the existing codes and the requirement.

                2.Design a NoSQL collection schema that satisfy the user requirement. You must only design ONE collection. Be concise, only includes the necessary properties in the collection. 

                3.Replace all the "todos" to the collection name you just created. Including directory names and files, fields names etc.
                OUTPUT FORMAT
                -----------------
                Step:[Step Number]
                Change Description: [Description]
